...
class InpaintingNetwork(nn.Module):
    ...
    def forward(self, source_image, dense_motion):
        ...
        output_dict['contribution_map32'] = dense_motion['contribution_map32']
        output_dict['contribution_map128'] = dense_motion['contribution_map128']
        output_dict['contribution_map256'] = dense_motion['contribution_map256']
        ...
        multideformation = dense_motion['multideformation']

        out_ij = self.deform_input(out.detach(), multideformation[0].detach())
        out = self.deform_input(out, multideformation[0])
        ...
        for i in range(self.num_down_blocks):
            ...
            #if (i == self.num_down_blocks - 2):
            encode_ij = self.deform_input(encode_i.detach(), multideformation[i+1].detach())
            encode_i = self.deform_input(encode_i, multideformation[i+1])
            #else:
            #    encode_ij = self.deform_input(encode_i.detach(), deformation)
            #    encode_i = self.deform_input(encode_i, deformation)
            #print('eeeeee',i,encode_i.shape)
            occlusion_ind = 0
            if self.multi_mask:
                occlusion_ind = i+1
        ...
        deformed_source = self.deform_input(source_image, multideformation[3])
        output_dict["deformed"] = deformed_source
        output_dict["warped_encoder_maps"] = warped_encoder_maps

        occlusion_last = occlusion_map[-1]
        if not self.multi_mask:
            occlusion_last = F.interpolate(occlusion_last, size=out.shape[2:], mode='bilinear',align_corners=True) #bilinear
