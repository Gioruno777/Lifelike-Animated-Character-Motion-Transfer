...
class DenseMotionNetwork(nn.Module):
    ...
    def __init__(self, block_expansion, num_blocks, max_features, num_tps, num_channels, 
        ...
        self.map32 = nn.Conv2d(hourglass_output_size[-2], num_tps + 1, kernel_size=(7, 7), padding=(3, 3))
        self.map128 = nn.Conv2d(hourglass_output_size[-1]//2, num_tps + 1, kernel_size=(7, 7), padding=(3, 3))
        self.map256 = nn.Conv2d(hourglass_output_size[-1] // 4, num_tps + 1, kernel_size=(7, 7), padding=(3, 3))
        ...
        if multi_mask:
            ...
            for i in range(self.up_nums):
                ...
                upt.append(UpBlock2d(channel[i], channel[i] // 2, kernel_size=3, padding=1))
            self.up = nn.ModuleList(up)
            self.upt = nn.ModuleList(upt)
        ...
        self.test = nn.Conv2d(64, 1, kernel_size=(7, 7), padding=(3, 3))
    ...
    def create_heatmap_representations1(self, source_image, kp_driving, kp_source):
        ...
        return heatmap,gaussian_driving,gaussian_source
    ...
    def multideform(self,size,transformations):
        transformations = transformations.permute(1, 0, 4, 2, 3)
        transformations_new = []
        for i in range(len(transformations)):
            #print(transformations[i].shape)
            x = F.interpolate(transformations[i],size=(size,size),mode='bilinear',align_corners=True).unsqueeze(0)
            x = x.permute(0, 1 ,3, 4, 2)
            transformations_new.append(x)
        flow = torch.cat(transformations_new, dim=1)
        #print('dddd',flow.shape)
        return flow
    ...
    def forward(self, source_image, kp_driving, kp_source, bg_param = None, dropout_flag=False, dropout_p = 0):
        ...
        heatmap_representation1 = self.create_heatmap_representations1(source_image, kp_driving, kp_source)
        out_dict['heatmap0'] = heatmap_representation1[0]
        out_dict['heatmap1'] = heatmap_representation1[1]
        out_dict['heatmap2'] = heatmap_representation1[2]
        ...
        #print(deformed_source.shape)
        input = torch.cat([heatmap_representation, deformed_source], dim=1)
        input = input.view(bs, -1, h, w)

        prediction = self.hourglass(input, mode = 1)
        #print('oooooooo',prediction[-1].shape)
        #print('oooooooo',prediction[0].shape,prediction[1].shape,prediction[2].shape,prediction[3].shape,prediction[4].shape)
        #print(self.hourglass.out_channels)

        contribution_maps = self.maps(prediction[-1])
        out_dict['transformation'] = transformations
        #print('oooooooo', contribution_maps.shape)
        if(dropout_flag):
        ...
        transformationss = transformations.permute(0, 1, 4, 2, 3)
        deformation = (transformationss * contribution_maps).sum(dim=1)
        deformation64 = deformation.permute(0, 2, 3, 1)
        out_dict['deformation'] = deformation64 # Optical Flow

        multideform = []
        transformation32 = self.multideform(32, transformations)
        out_dict['transformation32'] = transformation32
        contribution_map32 = self.map32(prediction[-2])
        if (dropout_flag):
            contribution_map32 = self.dropout_softmax(contribution_map32, dropout_p)
        else:
            contribution_map32 = F.softmax(contribution_map32, dim=1)
        out_dict['contribution_map32'] = contribution_map32
        contribution_map32 = contribution_map32.unsqueeze(2)
        transformation32 = transformation32.permute(0, 1, 4, 2, 3)
        deformation32 = (transformation32 * contribution_map32).sum(dim=1)
        deformation32 = deformation32.permute(0, 2, 3, 1)
        multideform.append(deformation32)
        multideform.append(deformation64)

        transformation128 = self.multideform(128, transformations)
        out_dict['transformation128'] = transformation128
        prediction128 = self.upt[0](prediction[-1])
        #print('128',prediction128.shape)
        contribution_map128 = self.map128(prediction128)
        if (dropout_flag):
            contribution_map128 = self.dropout_softmax(contribution_map128, dropout_p)
        else:
            contribution_map128 = F.softmax(contribution_map128, dim=1)
        out_dict['contribution_map128'] = contribution_map128
        contribution_map128 = contribution_map128.unsqueeze(2)
        transformation128 = transformation128.permute(0, 1, 4, 2, 3)
        deformation128 = (transformation128 * contribution_map128).sum(dim=1)
        deformation128 = deformation128.permute(0, 2, 3, 1)
        multideform.append(deformation128)

        transformation256 = self.multideform(256, transformations)
        out_dict['transformation256'] = transformation256
        prediction256 = self.upt[1](prediction128)
        contribution_map256 = self.map256(prediction256)
        if (dropout_flag):
            contribution_map256 = self.dropout_softmax(contribution_map256, dropout_p)
        else:
            contribution_map256 = F.softmax(contribution_map256, dim=1)
        out_dict['contribution_map256'] = contribution_map256
        contribution_map256 = contribution_map256.unsqueeze(2)
        transformation256 = transformation256.permute(0, 1, 4, 2, 3)
        deformation256 = (transformation256 * contribution_map256).sum(dim=1)
        deformation256 = deformation256.permute(0, 2, 3, 1)
        multideform.append(deformation256)

        out_dict['multideformation'] = multideform


        occlusion_map = []
        if self.multi_mask:
            for i in range(self.occlusion_num-self.up_nums):
                #print('1',self.up_nums-self.occlusion_num+i)
                occlusion_map.append(torch.sigmoid(self.occlusion[i](prediction[self.up_nums-self.occlusion_num+i]))) #sigmod
                #print('111111111',torch.sigmoid(self.occlusion[i](prediction[self.up_nums-self.occlusion_num+i])).shape)
            prediction = prediction[-1]
            for i in range(self.up_nums):
                #print('2', prediction.shape)
                prediction = self.up[i](prediction)
                occlusion_map.append(torch.sigmoid(self.occlusion[i+self.occlusion_num-self.up_nums](prediction))) #sigmoid
                #print('3',prediction.shape )
        else:
            occlusion_map.append(torch.sigmoid(self.occlusion[0](prediction[-1]))) #sigmoid
