    ...
    def visualize(self, driving, source, out):
        ...
        if 'deformed_source' in out:
            ...
            for i in range(out['deformed_source'].shape[1]):
                ...
                mask32 = out['contribution_map32'][:, i:(i + 1)].data.cpu().repeat(1, 3, 1, 1)
                mask = out['contribution_maps'][:, i:(i+1)].data.cpu().repeat(1, 3, 1, 1)
                mask128 = out['contribution_map128'][:, i:(i + 1)].data.cpu().repeat(1, 3, 1, 1)
                mask256 = out['contribution_map128'][:, i:(i + 1)].data.cpu().repeat(1, 3, 1, 1)

                mask32 = F.interpolate(mask32, size=source.shape[1:3])
                mask = F.interpolate(mask, size=source.shape[1:3])
                mask128 = F.interpolate(mask128, size=source.shape[1:3])
                mask256 = F.interpolate(mask256, size=source.shape[1:3])

                image = np.transpose(image.numpy(), (0, 2, 3, 1))
                mask32 = np.transpose(mask32.numpy(), (0, 2, 3, 1))
                mask = np.transpose(mask.numpy(), (0, 2, 3, 1))
                mask128 = np.transpose(mask128.numpy(), (0, 2, 3, 1))
                mask256 = np.transpose(mask256.numpy(), (0, 2, 3, 1))
                ...
                images.append(image)
                if i != 0:
                    images.append(mask32 * color)
                    images.append(mask * color)
                    images.append(mask128 * color)
                    images.append(mask256 * color)
                else:
                    images.append(mask32)
                    images.append(mask)
                    images.append(mask128 * color)
                    images.append(mask256 * color)

                full_mask32.append(mask32 * color)
                full_mask.append(mask * color)
                full_mask128.append(mask128 * color)
                full_mask256.append(mask256 * color)

            images.append(sum(full_mask32))
            images.append(sum(full_mask))
            images.append(sum(full_mask128))
            images.append(sum(full_mask256))
